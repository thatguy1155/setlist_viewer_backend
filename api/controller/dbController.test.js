...
describe('dbController Methods', () => {
    ...
    it('makes sure getDates returns the dates a song is played', async () => {
      ...
      await db.getArtist()
      expect(await db.getDates()).toEqual(sampleResult);
    });
    it('mocks the addData function', async () => {
      ...
      const pretendToAddAllInfo = (addedArtist,addSetlist,getSong) => {
        addedArtist()
        let listOfShows = db.apiResult.setlist
        for (const concert of listOfShows){
          ...
          if (concertSets.length > 0){
            let songsFromThisConcert = db.compileSongs(concertSets);
            console.log(songsFromThisConcert)
      ...
      pretendToAddAllInfo(addedArtist,addSetlist,getSong)
